...
特定の Pattern の文字列を表すための文字列表現。Pattern に Match する文字列を検索するために使用する。 Pattern を表すために Meta 文字と呼ばれる記号を使う。
...
Crawling, Scraping で Data を取得した後の Data 分析に強力な Library がある。 NumPy, SciPy という数値計算や科学技術計算の Library を Base にした Data分析用がある。
...
Browser の開発者 Tool で取得できる Xpath や CSS Selector を確認し、必要に応じて本質的なところだけ抽出したり使用する Selector を変更させながら活用すれば Simple かつ変化に強くなる。
...
Unix 系 OS の Default encoding は、UTF-8。 Python で File を開いて保存する場合も ```encoding=''``` を指定しないと UTF-8 で保存される。  
...
単純に File に保存するのに比べて、複数 Process から読み書きしやすく、Data の重複を防ぎやすくなる。後の行程で分析に利用する際、条件に合う１部 の Data だけ取り出すのも簡単。
...
- ある Program が File に書き込んでいる間は、他の Program からは同じ File に書き込めないように Lock されるので、複数 Program からの同時 書き込みにも向いていない。
...
- Primary key は上記の識別 Key とは別に、Surrogate key と呼ばれる Unique な値を生成して使用するのがよい。 ※ URL から取得した値は、Web site 側の Renewal
  などで変更する可能性がある。
...
HTML のみを解釈する Crawler に比べて１page あたりの処理に時間がかかり,Memory 消費が増える傾向。 JavaScript の実行が必要な Page に絞って使うなど工夫が必要。
...
自身の Crawler が Web server の処理能力を多く占めると他者が Web site を閲覧できなくなってしまう。商用 Site の場合、営業妨害となる可能性もあるので crawl 先に負荷をかけ過ぎないように配慮する。  
...
XML site map は、Web site の管理者が Crawler に対して crawl して欲しい URL の List を提示するための XML file 参照すると効率的。
...
余計な負荷をかけない行儀の良い Crawler を作るためには Error 処理も大切。 Web server が Access 過多という response を返しているのに何度も繰り返し request を送るといつまで経っても
access 過多の状態が解消されない。
...
Crawler の作成は、繰り返し実行することを念頭に置いて作成することが大事。

1. 更新された Data を取得できるようにするため
    - 更新された内容だけを crawl すれば素早く完了する
    - 相手の Server に与える負荷が減少する
2. Error などで停止した後に途中から再開できるようにするため
    - 様々な要因で Crawler が止まってしまっても続きから再開できるようにしておくと、途中までの crawl が無駄にならない。

### 更新された Data だけ取得する

- crawl 時にその日時を保存しておき、一定時間経過したら再度 crawl するように設計して最近 access していない Page のみを crawl するようにする。
- Web site によって更新頻度は異なるため、HTTP の cache policy に従う。

#### HTTP の cache

HTTP Server は、Response に下記表のような Cache に関する Header をつけることで、HTTP Client に対して Cache 方針を示せる。

| HTTP Header   | 強さ       | 説明                                                           |
|---------------|----------|--------------------------------------------------------------|
| Cache-Control | 強い Cache | Contents を cache しても良いかなど、Cache 方針を細かく指示する                   |
| Expires       | 強い Cache | Contents の有効期限を示す。                                           |
| ETag          | 弱い Cache | Contents の識別子を表す。Contents が変わると ETag の値も変わる。                 |
| Last-Modified | 弱い Cache | Contents の最終更新日時を表す。                                         |
| Pragma        |          | Cahe-Control と似たものだったが、現在では後方互換のためだけに残されている。                 |
| Vary          |          | 値に含まれる Request Header の値が変わると Server が返す response も変わることを表す。 |

##### 強い Cache
- Client は一度 response を cache すると有効期限が切れるまでは request を送らず、cache された Response を使用する。
- Server に負荷がかからない。

##### 弱い Cache
- Client は一度 response を cache すると次回からは条件つきの request を送り、Server は更新がない場合は **「304 Not Modified」** という
